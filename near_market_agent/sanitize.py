...
_INJECTION_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(r"you are now (?:a |an )?(?:new |different )?(?:ai|assistant|agent)", re.I),
        "[FILTERED]",
    ),
    ...
    (
        re.compile(
            r"respond (?:only )?with|output (?:only |exactly )?(?:the following|this)", re.I
        ),
        "[FILTERED]",
    ),
    (
        re.compile(r"your (?:new |real |actual )?(?:task|job|goal|objective|purpose) is", re.I),
        "[FILTERED]",
    ),
    # Encoding tricks
    ...
    (
        re.compile(
            r"(?:show|reveal|print|output|repeat) (?:your |the )?(?:system |full )?prompt", re.I
        ),
        "[FILTERED]",
    ),
